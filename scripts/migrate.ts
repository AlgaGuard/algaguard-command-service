...
try {
  await client.query(
    "SELECT pg_advisory_lock(hashtext('algaguard-command-migrations'))",
  );
  ...
  const filenames = (await readdir(path.resolve("migrations")))
  ...
  for (const filename of filenames) {
    ...
    const prior = await client.query(
      ...
      ["algaguard-command-service", filename],
    );
    if (prior.rows[0]) {
    ...
    try {
      ...
      await client.query(
        ...
        ["algaguard-command-service", filename, sha256],
      );
  ...
  await client.query(
    "SELECT pg_advisory_unlock(hashtext('algaguard-command-migrations'))",
  );
  client.release();
  await pool.end();
}
